...
def HMessages(f):
	...
	for name in f.order:
		v = f.features[name]
		if v["Category"] != "Deprecated" and v["FeatureType"] in [
			"fun",
			"get",
			"set",
		]:
...
def HEnumerations(f):
	...
	for name in f.order:
		v = f.features[name]
		if v["Category"] != "Deprecated" and v["FeatureType"] in ["evt"]:
			out.append("\t" + name + " = " + v["Value"] + ",")
	out.append("};")
...
def HConstants(f):
	...
	for _n, v in f.features.items():
		if v["Category"] != "Deprecated" and v["FeatureType"] in ["enu"]:
			allEnumPrefixes.extend(v["Value"].split())
	for name in f.order:
		v = f.features[name]
		if v["Category"] != "Deprecated" and v["FeatureType"] in ["val"]:
			hasPrefix = False
